...
public class TBStatusDataEvaluator implements PersonDataEvaluator {
	...
	public EvaluatedPersonData evaluate(PersonDataDefinition definition, EvaluationContext context)
		...
		String qry = "select client_id, CASE MID(MAX(CONCAT(encounter_datetime, tb_status)), 20) WHEN 'No Signs' THEN 'No Signs' WHEN 'Pr TB - Presumptive TB' THEN 'Presumptive TB' WHEN 'ND - TB Screening not done' THEN 'Not done' END "
		        + " AS tb_status FROM ssemr_etl.ssemr_flat_encounter_hiv_care_follow_up GROUP BY client_id, edd >= DATE(:endDate)";
